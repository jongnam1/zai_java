...
public class MainMt {
	...
	public static void main(String[] args) {
		...
		HashSet<String> set = new HashSet<>(); // 집합을 표현하기위한 용도 HashSet
	    ...
	    set.add("김유신"); 
	    System.out.println ( set ); // 중복저장 안됨
		
	    // HashSet은 직접 값 한개씩 출력 불가
	    // HashSet에 저장되어 있는 값 하나씩 출력 하는방법 1.
	    ArrayList<String> array = new ArrayList<>(set); // 출력할때 직접 하나씩 못뺌
	    
	    System.out.println(array.get(2));
	    
	    // 두번째 방법
/*인터페이스*/  Iterator<String> it = set.iterator(); // 하나씩하나씩 뺴오기 위한 전용인테페이스
             while(it.hasNext() ) {
             ...
             // 반복문 돌리면서 if문 사용해서 만원이하인 경우
             // hashSet A에 저장, 5만원 이하는 HashSet B에 저장
             // Hashset C에는 10만원 이하 저장
             
             set.remove("김유신");
             System.out.println( set ); //데이터 조회
             System.out.println( set.contains("강감찬")); // 데이터 찾기
             ...
             Iterator<String> 과일 =  찬양과일.iterator();   //일회용 
             while(과일.hasNext() ) {
             ...
             HashSet<String> 차집합2 = new HashSet<>(찬양과일); // 차집합구할 HashSet입력
